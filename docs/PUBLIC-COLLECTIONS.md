...
Le contrat additif `wp_seed_events_query_occurrence_collection()` retourne une entree par occurrence et permet de filtrer evenement, type, statut, epinglage, annulation et bornes temporelles.

Cette fonction ne remplace ni `[wp_seed_events]`, ni `wp_seed_events_query_event_collection()`. Sa route REST est `/wp-seed-events/v1/occurrences`. Voir [Occurrence Collections](OCCURRENCE-COLLECTIONS.md) et [Domain Model](../DOMAIN-MODEL.md).
